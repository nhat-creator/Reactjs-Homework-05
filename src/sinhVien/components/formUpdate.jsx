...
import { updateSinhVien } from "../../store/sinhVienReducer";
export default function FormUpdate() {
  const dispatch = useDispatch();
  const infoSinhVien = useSelector(
    (state) => state.SinhVienReducer.infoSinhVien
  );
  const [state, setState] = useState({
    values: {
      maSV: infoSinhVien.maSV,
      hoTen: infoSinhVien.hoTen,
      phoneNumber: infoSinhVien.phoneNumber,
      email: infoSinhVien.email,
    },
    errors: {
      maSV: "",
      hoTen: "",
  ...
  return (
    <div>
      <div className="max-w-4xl container mx-auto p-5 bg-white">
        <h1 className="font-bold text-2xl py-4 text-left text-yellow-600 border-b border-gray-200">
          <span className="text-red-600">(*)</span> Cập nhật thông tin sinh viên
        </h1>
        <form className="mt-6">
          ...
          <button
            ...
            onClick={(event) => {
              ...
              dispatch(updateSinhVien(state.values));
              setState({
                ...state,
                values: {
                  maSV: "",
